...
public:
    string smallestNumber(string num, long long t) {
        ...
        // No solution of the same length exists, so extend the number.
...
private:
    // Changed constexpr to const inline
    ...
    unordered_map<int, int> getFactorCount(const unordered_map<int, int>& count) {
        // Safe access helper with default value 0
        auto getVal = [](const unordered_map<int, int>& m, int k) {
            auto it = m.find(k);
            return it != m.end() ? it->second : 0;
        };

        int c2 = max(0, getVal(count, 2));
        int c3 = max(0, getVal(count, 3));
        int c5 = max(0, getVal(count, 5));
        int c7 = max(0, getVal(count, 7));

        const int count8 = c2 / 3;
        int remaining2 = c2 % 3;

        const int count9 = c3 / 2;
        int count3 = c3 % 2;
